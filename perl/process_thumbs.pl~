...
$dir_use = "/Users/solmon/Sites/data/" . @ARGV[0] . "/pngs/thmb"; 
...
## Creating thumbnails of fd images for mobile app
	...
	print  "Creating jpg thumbnails \n";
	        @args3 = ("/usr/local/bin/convert", $result_file, "-resize", "60x60", $result_file_60);
	        system(@args3) == 0 or die "Couldn't create thumbnail $base_file _small60.jpg, $!";
	        @args4 = ("/usr/local/bin/convert", $result_file, "-resize", "140x140", $result_file_140);
...
$dir_use = "/Users/solmon/Sites/data/" . @ARGV[0] . "/pngs"; 
#$dir_use = "./" . @ARGV[0] . "/pngs"; 
...
foreach $instname (sort readdir(dir)) {
	  ...
	  foreach $filename (sort readdir(Instfold)){
		...
		system("/usr/local/bin/convert -size 600x100 xc:none -font AvantGarde-Book -pointsize 50 -gravity center -stroke black -strokewidth 2 -annotate 0 'SolarMonitor.org' -background none -shadow 100x3+0+0 +repage -stroke none -fill white -annotate 0 'SolarMonitor.org' $file_use +swap -gravity southeast -geometry -90+50 -composite $result_file") == 0  || die "Could't create stamped image $base_file .png, $!";
		...
		$result_file = $inst_path . "/" . $base_file . ".png";
		system("/usr/local/bin/convert -size 600x100 xc:none -font AvantGarde-Book -pointsize 20 -gravity center -stroke black -strokewidth 2 -annotate 0 'SolarMonitor.org' -background none -shadow 100x3+0+0 +repage -stroke none -fill white -annotate 0 'SolarMonitor.org' $file_use +swap -gravity southeast -geometry -208+8 -composite $result_file") == 0 || die "Could't create stamped image $base_file .png, $!";
