...
; Read the active region summary for the requested and previous days.
    ...
    print, 'getting srs'        
    get_srs_repop, date_struct, srs_today, srs_yesterday, issued, t_noaa
    print, 'done getting srs'
	
; Get latest events from SSW database	    
 
    print, 'concating AR summary'
    last_events2arm2_repop, date_struct, events
...
; Concat AR summary and events list for today and yesterday
    ...
    if ( srs_today[ 0 ] ne 'No data' ) then begin
      print, 'doing ar comb'
      ar_comb_repop, date_struct, srs_today, srs_yesterday, events, summary, no_region_today, no_region_yesterday
...
;if not keyword_set(date_struct) then begin
if var_type(_extra) eq 8 then begin
	if test[0] eq 1 then perlthumb='process_thumbs_test.pl' else perlthumb='process_thumbs.pl'
...
if var_type(_extra) eq 8 and var_type(instrument_map_struct) eq 8 then begin
...
;TEMP !!! FOR EIT DATA
		mapdsz=(size(instrument_map_struct.scaled_map.data))[1:2]
		if mapdsz[0]+mapdsz[1] eq 2048 then $ 
			arm_regions, output_path, date_struct, summary, instrument_map_struct, _extra=_extra $
			else print,'ARM_REGIONS SKIPPED map data too small. '+strtrim(mapdsz[0])+'x'+strtrim(mapdsz[1])
;endelse
	endif
endif
