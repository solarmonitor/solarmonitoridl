...
if keyword_set(tec) then begin
	...
	spawn,'mv -f CTIPeTEC.png ../data/'+date+'/pngs/iono/tec_map_'+date+'.png'
	url2='http://helios.swpc.noaa.gov/ctipe/plots/CTIPeElectronDensity.png'
	sock_copy,url2,err=err
	spawn,'mv -f CTIPeElectronDensity.png ../data/'+date+'/pngs/iono/elec_map_'+date+'.png'
endif
if keyword_set(kyoto) then begin
	url='http://wdc.kugi.kyoto-u.ac.jp/ae_realtime/presentmonth/rtae_'+date+'.png'
	sock_copy,url,err=err;,copy_file=copy_file
	spawn,'mv -f rtae_'+date+'.png ../data/'+date+'/pngs/iono/kyoto_indices_'+date+'.png'
;get previous day:
	if not file_exist('../data/'+prevdate+'/pngs/iono/kyoto_indices_'+prevdate+'.png') then begin
		url='http://wdc.kugi.kyoto-u.ac.jp/ae_realtime/'+strmid(prevdate,0,6)+'/rtae_'+prevdate+'.png'
		sock_copy,url,err=err;,copy_file=copy_file
		spawn,'mv -f rtae_'+prevdate+'.png ../data/'+prevdate+'/pngs/iono/kyoto_indices_'+prevdate+'.png'
	endif
endif
if keyword_set(poes) then begin
	url='http://www.swpc.noaa.gov/pmap/gif/pmapN.gif'
	sock_copy,url,err=err;,copy_file=copy_file
	spawn,'mv -f pmapN.gif ../data/'+date+'/pngs/iono/poes_oval_'+date+'.png'
	url='http://www.swpc.noaa.gov/pmap/gif/pmapS.gif'
	sock_copy,url,err=err
	spawn,'mv -f pmapS.gif ../data/'+date+'/pngs/iono/poes_sout_'+date+'.png'
endif
if keyword_set(ovation) then begin
	url='http://sd-www.jhuapl.edu/Aurora/ovation_live/je_north_latest_oval.png'
	sock_copy,url,err=err
	spawn,'mv -f je_north_latest_oval.png ../data/'+date+'/pngs/iono/ovation_n_'+date+'.png'
	url='http://sd-www.jhuapl.edu/Aurora/ovation_live/je_south_latest_oval.png'
	sock_copy,url,err=err
	spawn,'mv -f je_south_latest_oval.png ../data/'+date+'/pngs/iono/ovation_s_'+date+'.png'
endif
