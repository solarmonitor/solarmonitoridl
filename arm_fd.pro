...
;Limb corrected Kanzelhohe.
	...
	;20130220 - PAH - grabs from Manuela's new archive
        get_kanzel_fast, date, filename, err
	if err eq 0 then kanzfast=1 else kanzfast=0

;BBSO archive.
	print,'Searching for BBSO data...'
	if (err eq -1) then $
		get_halpha, date, filename, err

;Try old Kanzel archive (searches back in time)
	if (err eq -1) then $
		get_kanzel, date, filename, err, exist, /today, /frfile
...
;   endif else begin
        ...
        if bbso eq 1 then begin
		  ...
		  bad_pixels = where( data gt 1e6 ) ;HIGGO 2012-06-06 -> changed from 1e4 to 1e6 since it was clipping the image
        ...
        if kanzel eq 1 and kanzfast eq 0 then begin
	...
	if kanzel eq 1 and kanzfast eq 0 then add_prop, map, data = bytscl( map.data, mdd*.3, mdd*1.5 ), /replace

	if kanzel eq 1 and kanzfast eq 1 then begin 
		map.data=map.data-min(map.data)
		add_prop, map, data = bytscl( map.data, mdd*.75, mdd*1.2 ), /replace
		;add_prop, map, data = bytscl( map.data,min(map.data),max(map.data) ), /replace
	endif
...
; SOLIS Chromosphere properties
  ...
  if ( keyword_set( slis_chrom ) ) then begin
	...
	;add_prop, map, dx = index.CDELT1*1d3, /replace ;HIGGO 2012-06-05 -> commented out since the observatory changed the units of their fits header
	;add_prop, map, dy = index.CDELT2*1d3, /replace ;HIGGO 2012-06-05 -> commented out since the observatory changed the units of their fits header
...
;    swap_obj -> latest
  ...
  if ( keyword_set( shmi_maglc ) ) then begin ;HIGGO 2012-06-08 -> Changed over to a new GET routine for HMI. Edited many lines in this section.

    print, 'Getting SDO HMI MAG'
;    get_sdo_jpg, map, instrument='_HMImag', filename=filename, err=err, /getmap,reso='full'

get_sdo_hmi_fits, filename, err=err, date=date
...
;    if err ne '' or var_type(map) ne 8 then begin
    ...
    add_prop, map, data = bytscl( data, min = -100, max = 100 ), /replace
