...
;               Updated to use SSW events database rather than webpage, Peter Gallagher, UCD
;
; Contact     : info@solarmonitor.org
;-

pro last_events2arm, date_struct, events
...
; Read the last_events web page.
  ...
  sock_ping, 'www.lmsal.com', up_down
  if ( up_down eq 0 ) then goto, get_out

  sock_list, 'www.lmsal.com/solarsoft/last_events/', page

; Identify C-class events from today and yesterday

  c_index = where( strmatch( page,'<td>C*' ) eq 1 )

  if ( c_index[ 0 ] ne -1 ) then begin

    c_mag  = strmid( page[ c_index ],     4, 4  ) 
    c_date = strmid( page[ c_index - 3 ], 4, 10 )
    c_time = strmid( page[ c_index - 3 ], 4, 19  )
    c_pos  = strmid( page[ c_index + 1 ], 4, 6  )
...
; Identify M-class events from today and yesterday
  ...
  m_index = where( strmatch( page,'<td>M*' ) eq 1 )

  if ( m_index[ 0 ] ne -1 ) then begin

    m_mag   = strmid( page[ m_index ],     4, 4  ) 
    m_date  = strmid( page[ m_index - 3 ], 4, 10 )
    m_time  = strmid( page[ m_index - 3 ], 4, 19  )
    m_pos  = strmid(  page[ m_index + 1 ], 4, 6  )
...
; Identify X-class events from today and yesterday
  ...
  x_index = where( strmatch( page,'<td>X*' ) eq 1 )

  if ( x_index[ 0 ] ne -1 ) then begin

    x_mag   = strmid( page[ x_index ],     4, 4  ) 
    x_date  = strmid( page[ x_index - 3 ], 4, 10 )
    x_time  = strmid( page[ x_index - 3 ], 4, 19  )
    x_pos  = strmid( page[  x_index + 1 ], 4, 6  )
