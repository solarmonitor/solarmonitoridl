...
label_neg=where(data lt 0,count)
...
;====classifies off limb CH regions========
			if (arccent0^(2)+arccent1^(2)) gt r_sun^(2) or (coord[0,xy(0,offs),0]^(2) + (coord[1,0,xy(1,offs)])^(2)) gt r_sun^(2) then begin
				subscripts=POLYFILLV(xy(0,offs + chpts ),xy(1,offs + chpts ),s[1],s[2])
				offarr[subscripts]=1
			endif else begin
...
;=====calculate area of CH with minimal projection effects======
				...
				trummar=truarcar*(6.96e+08/r_sun)*(6.96e+08/r_sun)
...
;====insertions of CH properties into property array=====
				...
				props[15,ident+1]=string(strcompress((arcar*100/(!PI*r_sun^(2))),/remove_all),format='(F10.1)')
...
if keyword_set(track) then begin
	...
	for i=0, n_elements(idloc)-2 do begin
		subscripts=POLYFILLV(predat.field1(0,idloc[i]+1:idloc[i+1]-1 ),predat.field1(1,idloc[i]+1:idloc[i+1]-1 ),s[1],s[2])
		preseg[subscripts]=predat.field1(1,idloc[i])
	endfor

	mxseg=predat.field1(1,idloc[n_elements(idloc)-1])
	dayb=anytim(strmid(head[0],13,25),/utime)
endif else begin
	preseg=lonarr(s[1],s[2])
...
daya=anytim(ind[1].date_obs,/utime)
openw,3,outpath+'/meta/arm_ch_location_'+strmid(time2file(map[1].time),0,8)+'.txt'
printf,3,'Date(utime): '+time2file(map[1].time,/seconds)
printf,3,'Solar_radius: '+string(r)
printf,3,'Pixel_2_arcsec: '+string(ind[1].cdelt1)
secs=daya-dayb
...
;====contour and label ch with tracked number========
				...
				for j=0L,(n_elements(info)-1) do begin
					if info[j].n le 100 then continue
					...
					offs=INFO[J].OFFSET
					if segarr[max(xy[0,offs+chpts])+1.,xy[1,offs+min(where((xy[0,offs+chpts]) eq max(xy[0,offs+chpts])))]] gt 0 then begin
						printf,3,'ID: '+string(0.)
					endif else begin
						printf,3,'ID: '+string(float(max(segarr)))
					endelse

					printf,3,string(xy(*,(offs + chpts)[0:*:10] ))
					printf,3,string(xy(*,offs + chpts[0] ))	
				endfor	
				cenarr[*]=0
...
;====contour and label CH boundary======
			...
			for j=0L,(n_elements(info)-1) do begin
				...
				if segarr[max(xy[0,offs+chpts])+1.,xy[1,offs+min(where((xy[0,offs+chpts]) eq max(xy[0,offs+chpts])))]] gt 0 then begin
					...
					printf,3,'ID: '+string(float(mxseg))
				endelse
				printf,3,string(xy(*,(offs + chpts)[0:*:10] ))
				printf,3,string(xy(*,offs + chpts[0] ))
			endfor	
...
printf,3,'MAX: '+string(float(mxseg))
close,3
close,2
